...
from venantvr.tools import setup_logging, configure_stream, dynamic_cache_to_json, cache_for_n_calls
...
class TestIntegration(unittest.TestCase):
    ...
    def test_logging_and_caching_integration(self):
        ...
        cache_file = Path(self.temp_dir) / "cache" / "test_exchange" / "get_market_data.json"
    ...
    def test_multiple_cache_instances(self):
        ...
        class ExchangeConnector:
            ...
            @dynamic_cache_to_json(f"{self.temp_dir}/{{exchange_name}}/{{api_version}}/")
        ...
        binance_cache = Path(self.temp_dir) / "binance" / "v1" / "get_exchange_info.json"
        coinbase_cache = Path(self.temp_dir) / "coinbase" / "v1" / "get_exchange_info.json"
